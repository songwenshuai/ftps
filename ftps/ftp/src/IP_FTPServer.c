...
typedef struct {
  const _FS_API          * pFS_API;       // File system info
...
*       _StoreUnsigned
*/
static char * _StoreUnsigned(char * pDest, uint32_t v, unsigned Base, int NumDigits) {
  unsigned Div;
  uint32_t Digit;
...
*       _WriteUnsigned
*/
static int _WriteUnsigned(OUT_BUFFER_CONTEXT * pOutContext, uint32_t v, unsigned Base, int NumDigits) {
  unsigned Div;
  uint32_t Digit;
  int r;
...
*/
static int _WriteUnsignedDataPort(FTPS_CONTEXT * pContext,uint32_t v, unsigned Base, int NumDigits) {
...
*/
static void _WriteFileTimeDataPort(FTPS_CONTEXT * pContext, uint32_t FileTime) {
  int Day;
  int Month;
  int Year;
  int Hour;
...
*       _SendFTPString
*/
static int _SendFTPString(OUT_BUFFER_CONTEXT * pOutContext, uint32_t Num, const char * s) {
  const char * sLine;
  char c;
  uint32_t Cnt;
...
static int _DeleteFile(FTPS_CONTEXT   * pContext, void * hFile) {
  return pContext->pFS_API->pfDeleteFile((const char*)hFile);
}
...
static int _ExecPASV(FTPS_CONTEXT * pContext) {
  ...
  //
  r = pContext->DataOut.pIP_API->pfAccept(pContext->CtrlOut.Sock, &pContext->DataOut.Sock);
  if (r == 0) {
    return 1;
  }
  return 0;
...
*/
int  IP_FTPS_Process (const IP_FTPS_API * pIP_API, FTPS_SOCKET CtrlSock, const _FS_API * pFS_API, const FTPS_APPLICATION * pApplication) {
  uint8_t acIn[FTPS_BUFFER_SIZE];
  uint8_t acOut[FTPS_BUFFER_SIZE];
...
*    Returns when the connection is closed or a fatal error occurs.
*/
void IP_FTPS_OnConnectionLimit(const IP_FTPS_API * pIP_API, FTPS_SOCKET CtrlSock) {
  uint8_t acOut[FTPS_ERR_BUFFER_SIZE];
